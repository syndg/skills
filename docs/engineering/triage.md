...
This is the most-filed gap on the skill, in three shapes. One is fully specified work waiting on another issue ([#139](https://github.com/mattpocock/skills/issues/139)); `ready-for-agent` is technically true there but misleading, so an agent can pick it up and hit a wall. Another is future work gated by an external trigger ([#297](https://github.com/mattpocock/skills/issues/297)). The third is a terminal "implemented, awaiting verification" state, without which an AFK runner can re-queue finished tickets. The blocked case is accepted as real, but the name (`blocked` or `paused`) remains undecided. None has shipped. A repository-local extra label can express the condition alongside the canonical state, although the skill will not understand it. Community variants add `needs-slicing`, `tracking`, and effort labels, but those are not part of this skill.
