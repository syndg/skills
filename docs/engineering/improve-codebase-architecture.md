...
- For designing one module you have already chosen, use [codebase-design](https://aihero.dev/skills-codebase-design): that is the bench, while this is the survey that finds what to put on it.
...
Yes. The exploration step asks for independent parallel subagents without naming one harness's tool or agent type. The harness only needs a way to run isolated exploration tasks and return their findings.
