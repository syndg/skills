...
It is this skill, renamed in v1.1. Practitioners were already pointing it at `AGENTS.md`, docs, specs, tickets, and runtime prompts before the name caught up. Structure, leading words, and pruning are the craft of any text an agent reads. There is no alias under the old name.
...
Ask which load you want to pay and which scope owns the instruction. Root `AGENTS.md` loads repository-wide; a child `AGENTS.md` narrows the instruction to a durable subtree and must appear in its parent's **Child DOX Index**. A harness-specific instruction file loads wherever that harness applies it. Material behind a pointer costs only the pointer line until its branch fires. Put applicable contracts in the nearest owner and disclose cold rationale instead of making every session pay for it.
...
This is a reach-for-it-anytime standalone reference. It sits underneath the whole set rather than beside one chain step. It governs the documents other skills leave behind: the root-to-nearest `AGENTS.md` hierarchy, co-located `DECISIONS.md` files after architectural decisions graduate from the hot path, configured DOX records, specs, and tickets. [unslop](https://aihero.dev/skills-unslop) is the cleanup pass beneath it: this skill owns document structure and behavioral precision, while `unslop` removes generic AI prose without changing those contracts. In a DOX project, `/dox` owns record retrieval and structural validation while [domain-modeling](https://aihero.dev/skills-domain-modeling) owns semantic updates. When you are unsure which skill or flow fits, [ask-matt](https://aihero.dev/skills-ask-matt) routes the set.
