...
Choose the contract branch before choosing the file. With `dox.config.json`, canonical contract and decision prose belongs in the applicable DOX record; do not duplicate it into `AGENTS.md` or `DECISIONS.md`. Without DOX, ask which load you want to pay and which scope owns the instruction: root `AGENTS.md` loads repository-wide, while a child narrows the instruction to a durable subtree and must appear in its parent's **Child DOX Index**. Harness-only operational instructions may still belong in the harness's own file. Material behind a pointer costs only the pointer line until its branch fires.
