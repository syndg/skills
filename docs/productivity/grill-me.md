...
- **A codebase to align against** — [grill-with-docs](https://aihero.dev/skills-grill-with-docs). The same interview, but [stateful](https://www.aihero.dev/ai-coding-dictionary/stateful): configured DOX projects use compact resolved items and update canonical records; unconfigured projects read the applicable root-to-nearest `AGENTS.md` contract and record settled language and decisions at the nearest owning scope.
...
Yes. Add this to the global instruction file your harness reads:
...
- You disagree with something. A session with no pushback from you is a session you did not need.
- Questions arrive in a few rounds rather than one long drip, and later rounds clearly build on what you said earlier.
- You end up somewhere you did not expect because a question surfaced an implicit decision.
- At the end, you could defend each choice to someone who was not there.
...
That portability is the whole difference from [grill-with-docs](https://aihero.dev/skills-grill-with-docs), which runs the same interview but aligns against a codebase and preserves settled knowledge in one selected contract store. In a configured DOX project, `/dox` returns compact items from canonical records and `domain-modeling` owns semantic updates. Only in the unconfigured fallback do terms go to the nearest owner's **Ubiquitous Language** and hard decisions join its inherited **Architectural Decisions**, with bodies graduating to a co-located `DECISIONS.md` when needed. Both front doors sit on the [grilling](https://aihero.dev/skills-grilling) primitive; `grill-me` is the stateless one.
