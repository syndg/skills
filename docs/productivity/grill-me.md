...
- **A codebase to align against** — [grill-with-docs](https://aihero.dev/skills-grill-with-docs). The same interview, but [stateful](https://www.aihero.dev/ai-coding-dictionary/stateful): configured projects follow the installed DOX skill for context and canonical-record maintenance; unconfigured projects use the applicable root-to-nearest `AGENTS.md` contract and record settled language and decisions at the nearest owner.
...
That portability is the difference from [grill-with-docs](https://aihero.dev/skills-grill-with-docs), which aligns the interview against a codebase and preserves settled knowledge in one contract store. Configured projects follow the installed DOX skill's retrieval and maintenance policy while `domain-modeling` adjudicates meaning. Only in the unconfigured fallback do terms go to the nearest owner's **Ubiquitous Language** and hard decisions join its inherited **Architectural Decisions**, with bodies graduating to co-located `DECISIONS.md` when needed. Both front doors sit on the [grilling](https://aihero.dev/skills-grilling) primitive; `grill-me` is the stateless one.
