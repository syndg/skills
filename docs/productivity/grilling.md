...
Yes, and a large part of the audience does. Add this to the global instruction file your harness reads:
...
Into this skill. Round-based questioning shipped briefly as a separate skill, then moved into `grilling`, so every wrapper got it at once. There is no separate `batch-grill-me` or sequential skill. The instruction above is the supported way back to one-at-a-time.
...
A confirmation gate exists precisely for this: the skill is not finished when the frontier empties; it is finished when you confirm the understanding is shared. Weaker and faster [models](https://www.aihero.dev/ai-coding-dictionary/model) still break it, most often by collapsing the interview into a couple of questions and an outline. Put a direct "do not implement without permission" instruction in the standing instruction file your harness reads if this recurs.
...
**`grill-me` is available, but invoking it does nothing.**
`grill-me` is a one-line wrapper whose whole body delegates to `/grilling`, so this primitive must be available too. `grill-with-docs` additionally delegates to [domain-modeling](https://aihero.dev/skills-domain-modeling). A selectively assembled skill set must include the primitives its wrappers call.
