...
Merely retrieving applicable project vocabulary is a one-line prose pointer, not the `domain-modeling` skill: follow the installed `/dox` skill's retrieval and reuse policy when `dox.config.json` exists, or read the applicable root-to-nearest `AGENTS.md` chain and indexed co-located `DECISIONS.md` entries only in the unconfigured fallback. Only the active build/sharpen discipline (challenge terms, test edge-case scenarios, adjudicate durable decisions, and update the canonical DOX record or fallback owning `AGENTS.md` / `DECISIONS.md`) is `domain-modeling`.
