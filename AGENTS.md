...
Repository-aware skills must preserve the DOX direct-cutover contract. Before repository research or code work, a configured project (`dox.config.json` present) resolves task- and path-specific context through `dox`; only an unconfigured project falls back to its applicable root-to-nearest `AGENTS.md` chain and any co-located `DECISIONS.md` entries that chain indexes. Reserve `CLAUDE.md` for harness-operational instructions and pointers, not fallback domain-contract storage. Never tell a configured project to enumerate the record directory, maintain a parallel `AGENTS.md`/`DECISIONS.md` decision ledger, or treat a receipt ID as loaded or portable contract content. Portable prompts and cross-worktree handoffs carry compact resolved items as hints and require the receiving worktree to resolve again.
...
- **Soft dependency** (`diagnosing-bugs`, `tdd`, `improve-codebase-architecture`): retrieve compact project contract items when DOX is configured, use the applicable root-to-nearest `AGENTS.md` chain and its indexed co-located `DECISIONS.md` entries only in the unconfigured fallback, and otherwise degrade gracefully when no project contract exists. Configured resolver or lint failures remain blockers; they never trigger an AGENTS fallback.
