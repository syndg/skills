...
1. **Inspect**
   - When `dox.config.json` exists, follow the installed `/dox` skill for retrieval eligibility, context reuse, and maintenance. Otherwise read the applicable root-to-nearest `AGENTS.md` chain and relevant indexed co-located `DECISIONS.md` entries as the repository contract. In both branches, read `CONTRIBUTING.md` for workflow instructions.
...
5. **Install and validate**
   - Follow the repository contract and `CONTRIBUTING.md` instructions loaded during inspection. Use the installed `/dox` skill's reuse policy for newly discovered scope; without DOX, read any applicable child `AGENTS.md` for those paths.
