...
- **Steering load**: should always-loaded instructions move into coding standards, DOX records, or other pointed-to references? Are ownership boundaries or Child DOX Index entries missing? _Use when_ root or child steering files are large, ambiguous, or repeatedly load unrelated material.
...
Use the repository's actual contract and review sources rather than assuming one universal layout:

- The applicable `AGENTS.md` hierarchy: always-loaded bindings and navigation pointers, kept small.
- DOX records and receipts when `dox.config.json` exists: project-local contracts, invariants, ownership, decisions, and proof obligations retrieved on demand.
- Co-located `DECISIONS.md`: cold storage for architectural decisions indexed by the owning `AGENTS.md`.
