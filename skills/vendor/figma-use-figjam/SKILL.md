...
> **MCP execution rules override reference snippets.** Some per-node references include examples written for the standalone plugin runtime. When using `use_figma`, follow `figma-use`: use top-level `await`, return results directly, and never wrap scripts in an async IIFE or call `figma.closePlugin()`.
