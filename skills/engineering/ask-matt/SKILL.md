...
- **`/dox`** is the automatic contract-retrieval and structural-validation layer when `dox.config.json` is present. It resolves once when repository work may be governed by recorded ownership, decisions, contracts, invariants, dependencies, or change impact that the immediate path does not reveal. After substantive repository content changes, it runs one changed-file impact review using the original task. It stays out of external tooling, runtime connectivity checks, Git-only actions, and ordinary follow-ups within an already-resolved task. Records constrain the work; source, tests, scripts, runbooks, configuration, and runtime inspection own current facts.
