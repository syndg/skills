...
- **Project language wins.** Preserve exact code symbols and canonical Ubiquitous Language. When `dox.config.json` exists, follow the installed `/dox` skill for retrieval eligibility, context reuse, and maintenance. Without DOX, use the applicable root-to-nearest `AGENTS.md` chain and its indexed co-located `DECISIONS.md` entries. A catalog word is not wrong when the project uses it precisely.
