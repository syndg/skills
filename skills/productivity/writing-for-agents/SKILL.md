...
description: Writing documents for agents. Use when creating or editing skills, AGENTS.md, CLAUDE.md, DOX records, plans, or any document an agent reaches through a context pointer.
---

Reference for writing any document an agent consumes: a skill, an `AGENTS.md` / `CLAUDE.md`, a DOX record, a plan, or a document reached by a pointer. The packaging differs; the writing does not: the same levers make each one predictable, since the agent takes the same _process_ every run rather than producing the same output.
