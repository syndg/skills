...
- **Revise when reality shifts, with confirmation.** Missions change, but do not silently rewrite the learner's goal. Confirm the proposed direction with the user first; then update this file and add a learning record explaining the change and its implications for future lessons.
