...
self-contained HTML file. Once duplication starts to hurt, the generic contract is to extract
reusable components into the workspace's `./assets/` directory and have lessons link to them.

**The kit is real, not a sketch.** It is one ready-made implementation of the `./assets/` contract,
implemented in
...
hand-writing a parallel CSS or navigation layer. A workspace with an established asset system may
keep that implementation instead.
...
When using the kit, each page is content-only: `<link rel="stylesheet" href="../assets/base.css">`, an empty
...
new lessons from it rather than restyling ad hoc. The included kit demonstrates this whole pattern:
a dark docs-app shell, amber accent, semantic meaning colors, and a manifest-driven rail + index.
